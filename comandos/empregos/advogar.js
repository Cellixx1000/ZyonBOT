...
module.exports = {
        run: async function (client, message, args) {
                try {
                    ...
                    let quimico = quimico1.val().medico
                    ...
                    let matlabItem = matlab1.val().estetoscopioItem
                    //<!-- Se ele não tiver, nós proibimos ele de usar o comando --!>
                    if (quimico == 0) return message.channel.send(`<a:erro:809516073799122945> | **${message.author.username}**, você não possui o emprego advogado. Caso você já possua 1,500 pontos de atividade, use: \`${prefixo}desbloquear advogado\`.`)
                    if (matlabItem == 0) return message.channel.send(`<a:erro:809516073799122945> | **${message.author.username}**, suas passagens aéreas acabaram. Visite a \`${prefixo}loja\` e compre mais algumas!`)
                    ...
                    if (author !== null && timeout - (Date.now() - author) > 0) {
                        ...
                        const mLenhador = new MessageEmbed()
                            .setTitle("👨‍⚖️ 8ª Vara do Juiz - Zyon:tm:")
                            .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
                            // Removido
                            .setThumbnail('https://i.imgur.com/dwQ5GID.png')
                            .setDescription(`Você visitou a 8ª Vara do Juiz e defendeu alguns clientes.`)
